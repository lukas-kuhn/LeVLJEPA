...
from utils.dataset import CC12MDataset
...
def main(cfg: DictConfig):
    ...
    dataset = CC12MDataset(tokenizer=tokenizer, max_samples=max_samples)
    if is_main:
    ...
    g1 = {
        "params": list(text_base.parameters()) + list(text_pre_proj.parameters()),
        "lr": LR,
    }
    g2 = {
        "params": list(vision_base.parameters()) + list(vision_pre_proj.parameters()),
        "lr": LR,
    }
    g3 = {"params": projector_vision.parameters(), "lr": LR}
    g4 = {"params": projector_text.parameters(), "lr": LR}

    optimizer = torch.optim.AdamW([g1, g2, g3, g4])
        ...
        with tqdm(range(TOTAL_STEPS), desc="Training", disable=not is_main) as t:
            for global_step in t:
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    text_raw = text_base(**text_inputs).last_hidden_state[
                        :, text_token_idx, :
                    ]
                    image_raw = vision_base(images)

                    image_linear = vision_pre_proj(image_raw)
                    text_linear = text_pre_proj(text_raw)

                    sigreg_vision = sigreg(image_linear)
                    sigreg_text = sigreg(text_linear)

                    image_proj = projector_vision(image_linear)
                    ...
                    mse_loss_cross_vision = (
                        (image_linear.detach() - text_proj).square().mean()
                    )

                    loss = (
                        (1 - (LAMBDA_VISION + LAMBDA_TEXT))
                        * (mse_loss_cross_text + mse_loss_cross_vision)
                        / 2
